...
{
	//d = new Distribution(0.0, 0.5, 0, 0.0);
	//multivariate gauss
	vec m;
...
void Agent::write_theta(string path, string filename)
{
	...
	ofstream file(path+filename, std::ios::binary | std::ios::out);
	std::ofstream os(filename.c_str(), std::ios::app);
	
    //file.write((char *) theta, MEMORY_SIZE * sizeof(double));
	os.write(reinterpret_cast<const char*>(theta), std::streamsize(MEMORY_SIZE * sizeof(double)));
