#include "../../include/environment/base.h"
    ...
    INSPECT_BOOKS(c["debug"]["inspect_books"].as<bool>(false))
{
	...
	else if (rm == "risk_based")
		reward = RewardMeasure::risk_based;
	else if (rm == "parametric_reward_function")
		reward = RewardMeasure::parametric_reward_function;
    else
	...
	if (tp != "midprice") {
		...
		//future_target_price = new market::tp::MidPrice(tp_lb);
	}
...
{
	//outfile.open("spread.txt", ios::out | ios::app);
    double r = 0.0;
    ...
    switch (reward) {
		...
		case RewardMeasure::risk_based:
			r = pnl_step - r_pos_weight * pow(e, momentum_pnl_step);
			break;

		case RewardMeasure::parametric_reward_function:
			r = w1 * (pnl_step)- w3 * max(0.0, momentum_pnl_step);
			//r += w2 * (1 / (spread_window.mean()));
			r += w2 * spread_window.mean();


			//to prove hypothesis
			//r += lo_vol_step;
			//r = r / 1000000.0;
			//outfile << std::to_string(spread_window.mean()) << "," << std::to_string(mpm) << endl;
			//outfile.close();
			break;
        ...
        case RewardMeasure::mm_linear:
			...
			/*if (momentum_pnl_step < 0)
				r = r_pnl_weight * pnl_step + r_pos_weight * momentum_pnl_step;
			else
				r = r_pnl_weight * pnl_step - r_pos_weight * momentum_pnl_step;*/
        ...
        case RewardMeasure::mm_exp:
			...
			//r = -exp(r_pos_weight * risk_manager_.exposure());
			//r = -exp(r_pos_weight * abs_pos);
...
/*--------market regime*/
void Base::setRegime(float reg) {
	mar_reg = reg;
}

float Base::getRegime() {
	return mar_reg;
}
...
bool Base::performAction(int action)
{
	...
	//access market regime and send to action function

    // Do agent's action -> risk manager
    DoAction(action, getRegime());//1 for neutral market regime
    ...
    do {
        ...
        double mpm = midprice_move(ask_book_, bid_book_);
		long agent_pos = risk_manager_.exposure();
		if (agent_pos < 0)
			int i = 0;
        pnl_step += agent_pos * mpm;
	...
	//cout << " Episode PnL :::" << experiment_stats.pnl << endl;
