#include "../../include/data/streamer.h"
...
template<typename R>
const R& Streamer<R>::Next_Record()
{
	return record_next;
}
...
//calls _LoadAllRowUntil(no of rows) in MarketDepth
template<typename R>
const vector<R> Streamer<R>::LoadAllUntil(int n)
{
	next_n_records.clear();
	_LoadAllRowUntil(n);
	return next_n_records;
}
