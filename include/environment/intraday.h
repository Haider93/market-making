...
class Intraday: public Base
{
    protected:
		...
		//preedictor vars
		long double next_state_target_price;
		deque<double> input_price_predictor;
		//PyObject *pName, *pModule, *pDict, *pFunc, *pArgs, *pValue, *pList, *pResult;
        ...
        void DoAction(int action, float regime);
        bool NextState();
    ...
    public:
		...
		//predictor function
		float predictor(deque<double> input_seq, int sliding_window_size);
};
}
