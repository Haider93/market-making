...
class Agent
{
    protected:
        ...
        Traces traces;
		EligibilityTrace eligibility_trace;

		//mixture of gaussian distributions over actions
		std::vector<Distribution> action_distributions;
		Distribution* d;
    ...
    public:
		...
		//univariate
		void update_action_dist(double update, int action);

		//multivariate
		void update_action_multi(double update, int action);

		//convert matrix to positive definite
		mat get_PD(mat m);

		//map<State&, double> state_q_map;
        ...
        // IO
        void write_theta(string path, string filename);
		double* get_theta(string filename);
};
